//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

//
// This source code was auto-generated by cf-sdk-builder
//

using CloudFoundry.CloudController.V2.Client;
using CloudFoundry.CloudController.V2.Client.Data;
using Microsoft.CSharp;
using Microsoft.VisualStudio.TestTools.UnitTesting;
using System.CodeDom.Compiler;

namespace CloudFoundry.CloudController.V2.Test.Deserialization
{
    [TestClass]
    [GeneratedCodeAttribute("cf-sdk-builder", "1.0.0.0")]
    public class EventsTest
    {


        [TestMethod]
        public void TestListAppCreateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""cfebd82d-944f-4b63-b12a-db437ecd0625"",
        ""url"": ""/v2/events/571589f4-88c0-4030-ab7b-121a57afad39"",
        ""created_at"": ""2016-09-02T11:52:18Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.app.create"",
        ""actor"": ""uaa-id-195"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""b7d86336-2dea-42f0-8daf-6da36db8e02b"",
        ""actee_type"": ""app"",
        ""actee_name"": ""name-1490"",
        ""timestamp"": ""2016-09-02T11:52:18Z"",
        ""metadata"": {
          ""request"": {
            ""name"": ""new"",
            ""instances"": 1,
            ""memory"": 84,
            ""state"": ""STOPPED"",
            ""environment_json"": ""PRIVATE DATA HIDDEN"",
            ""docker_credentials_json"": ""PRIVATE DATA HIDDEN""
          }
        },
        ""space_guid"": ""cfebd82d-944f-4b63-b12a-db437ecd0625"",
        ""organization_guid"": ""cfebd82d-944f-4b63-b12a-db437ecd0625""
      }
    }
  ]
}";

            PagedResponseCollection<ListAppCreateEventsResponse> page = Utilities.DeserializePage<ListAppCreateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("cfebd82d-944f-4b63-b12a-db437ecd0625", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/571589f4-88c0-4030-ab7b-121a57afad39", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:18Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.app.create", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-195", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("b7d86336-2dea-42f0-8daf-6da36db8e02b", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("app", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1490", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:18Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("cfebd82d-944f-4b63-b12a-db437ecd0625", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("cfebd82d-944f-4b63-b12a-db437ecd0625", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceDashboardClientDeleteEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""e9cd95ed-021e-45de-a5a8-a1bac8fdf046"",
        ""url"": ""/v2/events/b5a0d334-3a43-4cd6-8e00-23847ecc9470"",
        ""created_at"": ""2016-09-02T11:52:19Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_dashboard_client.delete"",
        ""actor"": ""1368b969-ee4b-4229-b7df-746852bfd267"",
        ""actor_type"": ""service_broker"",
        ""actor_name"": ""name-1625"",
        ""actee"": ""client_id"",
        ""actee_type"": ""service_dashboard_client"",
        ""actee_name"": ""client_id"",
        ""timestamp"": ""2016-09-02T11:52:19Z"",
        ""metadata"": {

        },
        ""space_guid"": """",
        ""organization_guid"": """"
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceDashboardClientDeleteEventsResponse> page = Utilities.DeserializePage<ListServiceDashboardClientDeleteEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("e9cd95ed-021e-45de-a5a8-a1bac8fdf046", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/b5a0d334-3a43-4cd6-8e00-23847ecc9470", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:19Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_dashboard_client.delete", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("1368b969-ee4b-4229-b7df-746852bfd267", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("service_broker", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("name-1625", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("client_id", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_dashboard_client", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("client_id", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:19Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceInstanceUnbindRouteEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""8beaaf6e-9731-4a82-bdc6-618aacd106b9"",
        ""url"": ""/v2/events/39a05a46-4c78-42f6-a24f-8333530fe52c"",
        ""created_at"": ""2016-09-02T11:52:20Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_instance.unbind_route"",
        ""actor"": ""uaa-id-219"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""3e3a57cf-39cc-4fcb-88e8-c81058dda542"",
        ""actee_type"": ""service_instance"",
        ""actee_name"": ""name-1821"",
        ""timestamp"": ""2016-09-02T11:52:20Z"",
        ""metadata"": {
          ""request"": {
            ""route_guid"": ""8beaaf6e-9731-4a82-bdc6-618aacd106b9"",
            ""parameters"": ""[PRIVATE DATA HIDDEN]""
          }
        },
        ""space_guid"": ""8beaaf6e-9731-4a82-bdc6-618aacd106b9"",
        ""organization_guid"": ""8beaaf6e-9731-4a82-bdc6-618aacd106b9""
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceInstanceUnbindRouteEventsResponse> page = Utilities.DeserializePage<ListServiceInstanceUnbindRouteEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("8beaaf6e-9731-4a82-bdc6-618aacd106b9", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/39a05a46-4c78-42f6-a24f-8333530fe52c", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:20Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_instance.unbind_route", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-219", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("3e3a57cf-39cc-4fcb-88e8-c81058dda542", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_instance", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1821", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:20Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("8beaaf6e-9731-4a82-bdc6-618aacd106b9", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("8beaaf6e-9731-4a82-bdc6-618aacd106b9", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceBrokerUpdateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""e158d033-9d5b-427d-8e19-4972cd066351"",
        ""url"": ""/v2/events/38154998-62c9-4ecf-9d8a-be5f187be95f"",
        ""created_at"": ""2016-09-02T11:52:16Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_broker.update"",
        ""actor"": ""uaa-id-173"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""69a1b331-3c8b-4047-b162-fb5143b5d057"",
        ""actee_type"": ""service_broker"",
        ""actee_name"": ""name-1192"",
        ""timestamp"": ""2016-09-02T11:52:16Z"",
        ""metadata"": {
          ""request"": {
            ""broker_url"": ""http://www.pancakes.com"",
            ""auth_password"": ""[REDACTED]""
          }
        },
        ""space_guid"": """",
        ""organization_guid"": """"
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceBrokerUpdateEventsResponse> page = Utilities.DeserializePage<ListServiceBrokerUpdateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("e158d033-9d5b-427d-8e19-4972cd066351", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/38154998-62c9-4ecf-9d8a-be5f187be95f", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:16Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_broker.update", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-173", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("69a1b331-3c8b-4047-b162-fb5143b5d057", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_broker", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1192", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:16Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServicePlanVisibilityDeleteEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""a0345f5c-1768-4847-be1b-3ac530ec6512"",
        ""url"": ""/v2/events/77fd0498-332c-430e-9b95-c57878d3fc19"",
        ""created_at"": ""2016-09-02T11:52:21Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_plan_visibility.delete"",
        ""actor"": ""uaa-id-237"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""e56d5633-b1ac-44c2-a96f-76bb385be052"",
        ""actee_type"": ""service_plan_visibility"",
        ""actee_name"": """",
        ""timestamp"": ""2016-09-02T11:52:21Z"",
        ""metadata"": {
          ""request"": {

          }
        },
        ""space_guid"": """",
        ""organization_guid"": ""a0345f5c-1768-4847-be1b-3ac530ec6512""
      }
    }
  ]
}";

            PagedResponseCollection<ListServicePlanVisibilityDeleteEventsResponse> page = Utilities.DeserializePage<ListServicePlanVisibilityDeleteEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("a0345f5c-1768-4847-be1b-3ac530ec6512", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/77fd0498-332c-430e-9b95-c57878d3fc19", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_plan_visibility.delete", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-237", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("e56d5633-b1ac-44c2-a96f-76bb385be052", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_plan_visibility", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("a0345f5c-1768-4847-be1b-3ac530ec6512", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListSpaceCreateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""a161fd0d-295a-4317-a685-52c7bf78d6c7"",
        ""url"": ""/v2/events/675fe22e-a59b-4100-bd52-590e9501f4e8"",
        ""created_at"": ""2016-09-02T11:52:18Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.space.create"",
        ""actor"": ""uaa-id-193"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""09a44f84-98e9-4a12-a876-71ff2da3af12"",
        ""actee_type"": ""space"",
        ""actee_name"": ""name-1463"",
        ""timestamp"": ""2016-09-02T11:52:18Z"",
        ""metadata"": {
          ""request"": {
            ""name"": ""outer space""
          }
        },
        ""space_guid"": ""a161fd0d-295a-4317-a685-52c7bf78d6c7"",
        ""organization_guid"": ""a161fd0d-295a-4317-a685-52c7bf78d6c7""
      }
    }
  ]
}";

            PagedResponseCollection<ListSpaceCreateEventsResponse> page = Utilities.DeserializePage<ListSpaceCreateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("a161fd0d-295a-4317-a685-52c7bf78d6c7", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/675fe22e-a59b-4100-bd52-590e9501f4e8", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:18Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.space.create", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-193", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("09a44f84-98e9-4a12-a876-71ff2da3af12", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("space", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1463", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:18Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("a161fd0d-295a-4317-a685-52c7bf78d6c7", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("a161fd0d-295a-4317-a685-52c7bf78d6c7", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListAppSshAuthorizedEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""a65c03e2-ae63-483e-bb6c-0b2a69e6b87a"",
        ""url"": ""/v2/events/d7ed9020-fc8d-4636-abed-9b44e39f853a"",
        ""created_at"": ""2016-09-02T11:52:16Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.app.ssh-authorized"",
        ""actor"": ""uaa-id-169"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""8c8e30b4-4174-4770-abe3-45c843e3fe63"",
        ""actee_type"": ""app"",
        ""actee_name"": ""name-1136"",
        ""timestamp"": ""2016-09-02T11:52:16Z"",
        ""metadata"": {
          ""index"": 1
        },
        ""space_guid"": ""a65c03e2-ae63-483e-bb6c-0b2a69e6b87a"",
        ""organization_guid"": ""a65c03e2-ae63-483e-bb6c-0b2a69e6b87a""
      }
    }
  ]
}";

            PagedResponseCollection<ListAppSshAuthorizedEventsResponse> page = Utilities.DeserializePage<ListAppSshAuthorizedEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("a65c03e2-ae63-483e-bb6c-0b2a69e6b87a", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/d7ed9020-fc8d-4636-abed-9b44e39f853a", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:16Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.app.ssh-authorized", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-169", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("8c8e30b4-4174-4770-abe3-45c843e3fe63", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("app", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1136", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:16Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("a65c03e2-ae63-483e-bb6c-0b2a69e6b87a", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("a65c03e2-ae63-483e-bb6c-0b2a69e6b87a", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListUserProvidedServiceInstanceCreateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""7762aa0c-5050-4d8f-a462-dcb0c5db88f9"",
        ""url"": ""/v2/events/14ce2e25-bfb6-4630-b7fd-45d69f20ccc6"",
        ""created_at"": ""2016-09-02T11:52:19Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.user_provided_service_instance.create"",
        ""actor"": ""uaa-id-211"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""ce144e26-65f0-41f0-a9ac-f4e4796cfc44"",
        ""actee_type"": ""user_provided_service_instance"",
        ""actee_name"": ""name-1705"",
        ""timestamp"": ""2016-09-02T11:52:19Z"",
        ""metadata"": {
          ""request"": {
            ""name"": ""name-1705"",
            ""space_guid"": ""7762aa0c-5050-4d8f-a462-dcb0c5db88f9""
          }
        },
        ""space_guid"": ""7762aa0c-5050-4d8f-a462-dcb0c5db88f9"",
        ""organization_guid"": ""7762aa0c-5050-4d8f-a462-dcb0c5db88f9""
      }
    }
  ]
}";

            PagedResponseCollection<ListUserProvidedServiceInstanceCreateEventsResponse> page = Utilities.DeserializePage<ListUserProvidedServiceInstanceCreateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("7762aa0c-5050-4d8f-a462-dcb0c5db88f9", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/14ce2e25-bfb6-4630-b7fd-45d69f20ccc6", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:19Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.user_provided_service_instance.create", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-211", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("ce144e26-65f0-41f0-a9ac-f4e4796cfc44", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("user_provided_service_instance", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1705", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:19Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("7762aa0c-5050-4d8f-a462-dcb0c5db88f9", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("7762aa0c-5050-4d8f-a462-dcb0c5db88f9", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceInstanceDeleteEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""3f675edf-ed8a-437b-8b90-d45978b50908"",
        ""url"": ""/v2/events/1f9364cc-7f49-4f12-8440-3adabc34623b"",
        ""created_at"": ""2016-09-02T11:52:17Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_instance.delete"",
        ""actor"": ""uaa-id-183"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""55348fe9-41b6-493d-8064-94cb245b7f4c"",
        ""actee_type"": ""service_instance"",
        ""actee_name"": ""name-1322"",
        ""timestamp"": ""2016-09-02T11:52:17Z"",
        ""metadata"": {
          ""request"": {
            ""parameters"": ""[PRIVATE DATA HIDDEN]""
          }
        },
        ""space_guid"": ""3f675edf-ed8a-437b-8b90-d45978b50908"",
        ""organization_guid"": ""3f675edf-ed8a-437b-8b90-d45978b50908""
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceInstanceDeleteEventsResponse> page = Utilities.DeserializePage<ListServiceInstanceDeleteEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("3f675edf-ed8a-437b-8b90-d45978b50908", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/1f9364cc-7f49-4f12-8440-3adabc34623b", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:17Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_instance.delete", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-183", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("55348fe9-41b6-493d-8064-94cb245b7f4c", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_instance", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1322", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:17Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("3f675edf-ed8a-437b-8b90-d45978b50908", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("3f675edf-ed8a-437b-8b90-d45978b50908", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceCreateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""845661ef-fa48-4fc3-890e-a48f30311513"",
        ""url"": ""/v2/events/6003b8c2-af58-41b0-81e5-fd8608ccfb46"",
        ""created_at"": ""2016-09-02T11:52:19Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service.create"",
        ""actor"": ""0b99f822-0500-48b7-841f-674dfddf49c2"",
        ""actor_type"": ""service_broker"",
        ""actor_name"": ""name-1680"",
        ""actee"": ""guid"",
        ""actee_type"": ""service"",
        ""actee_name"": ""label"",
        ""timestamp"": ""2016-09-02T11:52:19Z"",
        ""metadata"": {
          ""label"": ""label"",
          ""provider"": null,
          ""url"": null,
          ""description"": ""BOOOO"",
          ""long_description"": null,
          ""version"": null,
          ""info_url"": null,
          ""active"": true,
          ""bindable"": true,
          ""unique_id"": null,
          ""extra"": null,
          ""tags"": [

          ],
          ""requires"": [

          ],
          ""documentation_url"": null,
          ""service_broker_guid"": ""845661ef-fa48-4fc3-890e-a48f30311513"",
          ""plan_updateable"": false
        },
        ""space_guid"": """",
        ""organization_guid"": """"
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceCreateEventsResponse> page = Utilities.DeserializePage<ListServiceCreateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("845661ef-fa48-4fc3-890e-a48f30311513", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/6003b8c2-af58-41b0-81e5-fd8608ccfb46", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:19Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service.create", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("0b99f822-0500-48b7-841f-674dfddf49c2", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("service_broker", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("name-1680", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("guid", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("label", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:19Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceInstanceUpdateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""dc54f23b-783e-4787-b067-28838dbfaf0e"",
        ""url"": ""/v2/events/cc74f882-1f08-4d61-aacc-91f6f67c8eed"",
        ""created_at"": ""2016-09-02T11:52:19Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_instance.update"",
        ""actor"": ""uaa-id-207"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""de2bb73b-1554-41dc-acb7-698ad25ed9f9"",
        ""actee_type"": ""service_instance"",
        ""actee_name"": ""name-1650"",
        ""timestamp"": ""2016-09-02T11:52:19Z"",
        ""metadata"": {
          ""request"": {
            ""service_plan_guid"": ""dc54f23b-783e-4787-b067-28838dbfaf0e"",
            ""parameters"": ""[PRIVATE DATA HIDDEN]""
          }
        },
        ""space_guid"": ""dc54f23b-783e-4787-b067-28838dbfaf0e"",
        ""organization_guid"": ""dc54f23b-783e-4787-b067-28838dbfaf0e""
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceInstanceUpdateEventsResponse> page = Utilities.DeserializePage<ListServiceInstanceUpdateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("dc54f23b-783e-4787-b067-28838dbfaf0e", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/cc74f882-1f08-4d61-aacc-91f6f67c8eed", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:19Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_instance.update", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-207", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("de2bb73b-1554-41dc-acb7-698ad25ed9f9", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_instance", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1650", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:19Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("dc54f23b-783e-4787-b067-28838dbfaf0e", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("dc54f23b-783e-4787-b067-28838dbfaf0e", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceInstanceBindRouteEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""ce0d3954-9e79-4495-8c72-4f336fc51629"",
        ""url"": ""/v2/events/2eccc677-62e0-4cae-875f-ba2e70b378f7"",
        ""created_at"": ""2016-09-02T11:52:20Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_instance.bind_route"",
        ""actor"": ""uaa-id-225"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""1de15add-8035-4246-b02d-5b4efcb92a05"",
        ""actee_type"": ""service_instance"",
        ""actee_name"": ""name-1909"",
        ""timestamp"": ""2016-09-02T11:52:20Z"",
        ""metadata"": {
          ""request"": {
            ""route_guid"": ""ce0d3954-9e79-4495-8c72-4f336fc51629"",
            ""parameters"": ""[PRIVATE DATA HIDDEN]""
          }
        },
        ""space_guid"": ""ce0d3954-9e79-4495-8c72-4f336fc51629"",
        ""organization_guid"": ""ce0d3954-9e79-4495-8c72-4f336fc51629""
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceInstanceBindRouteEventsResponse> page = Utilities.DeserializePage<ListServiceInstanceBindRouteEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("ce0d3954-9e79-4495-8c72-4f336fc51629", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/2eccc677-62e0-4cae-875f-ba2e70b378f7", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:20Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_instance.bind_route", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-225", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("1de15add-8035-4246-b02d-5b4efcb92a05", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_instance", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1909", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:20Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("ce0d3954-9e79-4495-8c72-4f336fc51629", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("ce0d3954-9e79-4495-8c72-4f336fc51629", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListAppStopEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""71ed0565-2338-4a84-af65-12b2d47df62c"",
        ""url"": ""/v2/events/2d11a97e-7ce7-4b6b-8c67-accd5545b25f"",
        ""created_at"": ""2016-09-02T11:52:22Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.app.stop"",
        ""actor"": ""uaa-id-243"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""a1ff2633-c567-4f33-93a2-ebc26bb77410"",
        ""actee_type"": ""v3-app"",
        ""actee_name"": ""name-2172"",
        ""timestamp"": ""2016-09-02T11:52:22Z"",
        ""metadata"": {

        },
        ""space_guid"": ""71ed0565-2338-4a84-af65-12b2d47df62c"",
        ""organization_guid"": ""71ed0565-2338-4a84-af65-12b2d47df62c""
      }
    }
  ]
}";

            PagedResponseCollection<ListAppStopEventsResponse> page = Utilities.DeserializePage<ListAppStopEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("71ed0565-2338-4a84-af65-12b2d47df62c", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/2d11a97e-7ce7-4b6b-8c67-accd5545b25f", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:22Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.app.stop", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-243", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("a1ff2633-c567-4f33-93a2-ebc26bb77410", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("v3-app", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-2172", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:22Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("71ed0565-2338-4a84-af65-12b2d47df62c", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("71ed0565-2338-4a84-af65-12b2d47df62c", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestRetrieveEventResponse()
        {
            string json = @"{
  ""metadata"": {
    ""guid"": ""dfaf3e8c-bfcc-41be-a462-f038faf94373"",
    ""url"": ""/v2/events/d0c93b7d-1772-4831-bae8-afb75fe46e3a"",
    ""created_at"": ""2016-09-02T11:52:15Z"",
    ""updated_at"": null
  },
  ""entity"": {
    ""type"": ""name-1000"",
    ""actor"": ""2d2cfd4d-b705-49b8-9514-7cf110edca5b"",
    ""actor_type"": ""name-1001"",
    ""actor_name"": ""name-1002"",
    ""actee"": ""6e633146-6808-46b3-b63e-9aefc3f36c3c"",
    ""actee_type"": ""name-1003"",
    ""actee_name"": ""name-1004"",
    ""timestamp"": ""2016-09-02T11:52:15Z"",
    ""metadata"": {

    },
    ""space_guid"": ""dfaf3e8c-bfcc-41be-a462-f038faf94373"",
    ""organization_guid"": ""dfaf3e8c-bfcc-41be-a462-f038faf94373""
  }
}";

            RetrieveEventResponse obj = Utilities.DeserializeJson<RetrieveEventResponse>(json);

            Assert.AreEqual("dfaf3e8c-bfcc-41be-a462-f038faf94373", TestUtil.ToTestableString(obj.EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/d0c93b7d-1772-4831-bae8-afb75fe46e3a", TestUtil.ToTestableString(obj.EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:15Z", TestUtil.ToTestableString(obj.EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(obj.EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("name-1000", TestUtil.ToTestableString(obj.Type), true);
            Assert.AreEqual("2d2cfd4d-b705-49b8-9514-7cf110edca5b", TestUtil.ToTestableString(obj.Actor), true);
            Assert.AreEqual("name-1001", TestUtil.ToTestableString(obj.ActorType), true);
            Assert.AreEqual("name-1002", TestUtil.ToTestableString(obj.ActorName), true);
            Assert.AreEqual("6e633146-6808-46b3-b63e-9aefc3f36c3c", TestUtil.ToTestableString(obj.Actee), true);
            Assert.AreEqual("name-1003", TestUtil.ToTestableString(obj.ActeeType), true);
            Assert.AreEqual("name-1004", TestUtil.ToTestableString(obj.ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:15Z", TestUtil.ToTestableString(obj.Timestamp), true);
            Assert.AreEqual("dfaf3e8c-bfcc-41be-a462-f038faf94373", TestUtil.ToTestableString(obj.SpaceGuid), true);
            Assert.AreEqual("dfaf3e8c-bfcc-41be-a462-f038faf94373", TestUtil.ToTestableString(obj.OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListRouteUpdateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""1b3fedce-b2ca-47ec-9f40-67f315859c34"",
        ""url"": ""/v2/events/0fa3a3ec-fbf2-4dcd-b81b-e1fcdf5089f4"",
        ""created_at"": ""2016-09-02T11:52:18Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.route.update"",
        ""actor"": ""uaa-id-189"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""d6f5f6d4-0215-42e1-8551-a126896336c1"",
        ""actee_type"": ""route"",
        ""actee_name"": ""host-22"",
        ""timestamp"": ""2016-09-02T11:52:18Z"",
        ""metadata"": {
          ""request"": {
            ""host"": ""dora"",
            ""domain_guid"": ""1b3fedce-b2ca-47ec-9f40-67f315859c34"",
            ""space_guid"": ""1b3fedce-b2ca-47ec-9f40-67f315859c34""
          }
        },
        ""space_guid"": ""1b3fedce-b2ca-47ec-9f40-67f315859c34"",
        ""organization_guid"": ""1b3fedce-b2ca-47ec-9f40-67f315859c34""
      }
    }
  ]
}";

            PagedResponseCollection<ListRouteUpdateEventsResponse> page = Utilities.DeserializePage<ListRouteUpdateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("1b3fedce-b2ca-47ec-9f40-67f315859c34", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/0fa3a3ec-fbf2-4dcd-b81b-e1fcdf5089f4", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:18Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.route.update", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-189", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("d6f5f6d4-0215-42e1-8551-a126896336c1", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("route", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("host-22", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:18Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("1b3fedce-b2ca-47ec-9f40-67f315859c34", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("1b3fedce-b2ca-47ec-9f40-67f315859c34", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceUpdateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""837e9ede-b5bb-4709-a195-7acacc89bca8"",
        ""url"": ""/v2/events/948b38ae-f800-47cd-847f-72950ad07e9b"",
        ""created_at"": ""2016-09-02T11:52:17Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service.update"",
        ""actor"": ""6b19e7f4-1512-40a3-810c-36d1d4fee602"",
        ""actor_type"": ""service_broker"",
        ""actor_name"": ""name-1243"",
        ""actee"": ""559df812-6e20-4279-aa5c-5c147792c453"",
        ""actee_type"": ""service"",
        ""actee_name"": ""new label"",
        ""timestamp"": ""2016-09-02T11:52:17Z"",
        ""metadata"": {
          ""label"": ""new label""
        },
        ""space_guid"": """",
        ""organization_guid"": """"
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceUpdateEventsResponse> page = Utilities.DeserializePage<ListServiceUpdateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("837e9ede-b5bb-4709-a195-7acacc89bca8", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/948b38ae-f800-47cd-847f-72950ad07e9b", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:17Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service.update", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("6b19e7f4-1512-40a3-810c-36d1d4fee602", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("service_broker", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("name-1243", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("559df812-6e20-4279-aa5c-5c147792c453", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("new label", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:17Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceDashboardClientCreateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""c34b0662-1884-43ee-baf0-12a8339915a2"",
        ""url"": ""/v2/events/37b05eaf-5188-4cd0-99bb-d02ea1f0fab3"",
        ""created_at"": ""2016-09-02T11:52:18Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_dashboard_client.create"",
        ""actor"": ""0be57af9-808d-4dbe-bc3f-47d335011dfd"",
        ""actor_type"": ""service_broker"",
        ""actor_name"": ""name-1438"",
        ""actee"": ""client_id"",
        ""actee_type"": ""service_dashboard_client"",
        ""actee_name"": ""client_id"",
        ""timestamp"": ""2016-09-02T11:52:18Z"",
        ""metadata"": {
          ""secret"": ""[REDACTED]"",
          ""redirect_uri"": ""example.com/redirect""
        },
        ""space_guid"": """",
        ""organization_guid"": """"
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceDashboardClientCreateEventsResponse> page = Utilities.DeserializePage<ListServiceDashboardClientCreateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("c34b0662-1884-43ee-baf0-12a8339915a2", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/37b05eaf-5188-4cd0-99bb-d02ea1f0fab3", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:18Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_dashboard_client.create", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("0be57af9-808d-4dbe-bc3f-47d335011dfd", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("service_broker", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("name-1438", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("client_id", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_dashboard_client", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("client_id", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:18Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListSpaceDeleteEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""5e22104b-2a18-43dc-97e0-12c0fb1ab1eb"",
        ""url"": ""/v2/events/b9d74d94-a8f7-441f-b523-82e9a5b38bb9"",
        ""created_at"": ""2016-09-02T11:52:17Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.space.delete-request"",
        ""actor"": ""uaa-id-179"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""b6c3ea51-b577-437f-8b25-f1c51b76b4e5"",
        ""actee_type"": ""space"",
        ""actee_name"": ""name-1268"",
        ""timestamp"": ""2016-09-02T11:52:17Z"",
        ""metadata"": {
          ""request"": {
            ""recursive"": true
          }
        },
        ""space_guid"": ""5e22104b-2a18-43dc-97e0-12c0fb1ab1eb"",
        ""organization_guid"": ""5e22104b-2a18-43dc-97e0-12c0fb1ab1eb""
      }
    }
  ]
}";

            PagedResponseCollection<ListSpaceDeleteEventsResponse> page = Utilities.DeserializePage<ListSpaceDeleteEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("5e22104b-2a18-43dc-97e0-12c0fb1ab1eb", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/b9d74d94-a8f7-441f-b523-82e9a5b38bb9", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:17Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.space.delete-request", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-179", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("b6c3ea51-b577-437f-8b25-f1c51b76b4e5", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("space", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1268", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:17Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("5e22104b-2a18-43dc-97e0-12c0fb1ab1eb", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("5e22104b-2a18-43dc-97e0-12c0fb1ab1eb", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServicePlanVisibilityUpdateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""c826eab9-a996-46e0-9585-481e1c0b45da"",
        ""url"": ""/v2/events/7c6d6ab6-c719-4967-869c-d43f2616ddd7"",
        ""created_at"": ""2016-09-02T11:52:17Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_plan_visibility.update"",
        ""actor"": ""uaa-id-185"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""e4351c8e-6d5b-4850-a211-fa1019d22f1a"",
        ""actee_type"": ""service_plan_visibility"",
        ""actee_name"": """",
        ""timestamp"": ""2016-09-02T11:52:17Z"",
        ""metadata"": {
          ""request"": {
            ""service_plan_guid"": ""c826eab9-a996-46e0-9585-481e1c0b45da"",
            ""organization_guid"": ""c826eab9-a996-46e0-9585-481e1c0b45da""
          }
        },
        ""space_guid"": """",
        ""organization_guid"": ""c826eab9-a996-46e0-9585-481e1c0b45da""
      }
    }
  ]
}";

            PagedResponseCollection<ListServicePlanVisibilityUpdateEventsResponse> page = Utilities.DeserializePage<ListServicePlanVisibilityUpdateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("c826eab9-a996-46e0-9585-481e1c0b45da", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/7c6d6ab6-c719-4967-869c-d43f2616ddd7", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:17Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_plan_visibility.update", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-185", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("e4351c8e-6d5b-4850-a211-fa1019d22f1a", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_plan_visibility", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:17Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("c826eab9-a996-46e0-9585-481e1c0b45da", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceBrokerDeleteEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""11ffe3d7-71e4-4728-b245-c661f7cabde2"",
        ""url"": ""/v2/events/ea9e9994-33f2-4750-bf01-8cd916c32f69"",
        ""created_at"": ""2016-09-02T11:52:22Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_broker.delete"",
        ""actor"": ""uaa-id-241"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""bbcf30cf-0703-4c19-b825-d2eaf22df0b8"",
        ""actee_type"": ""service_broker"",
        ""actee_name"": ""name-2147"",
        ""timestamp"": ""2016-09-02T11:52:22Z"",
        ""metadata"": {
          ""request"": {

          }
        },
        ""space_guid"": """",
        ""organization_guid"": """"
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceBrokerDeleteEventsResponse> page = Utilities.DeserializePage<ListServiceBrokerDeleteEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("11ffe3d7-71e4-4728-b245-c661f7cabde2", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/ea9e9994-33f2-4750-bf01-8cd916c32f69", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:22Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_broker.delete", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-241", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("bbcf30cf-0703-4c19-b825-d2eaf22df0b8", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_broker", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-2147", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:22Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceBindingCreateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""a2268aa9-6b19-4603-84f0-aa0bbe2485f0"",
        ""url"": ""/v2/events/02f311b8-6f3c-472e-9b81-ab7a0deee590"",
        ""created_at"": ""2016-09-02T11:52:19Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_binding.create"",
        ""actor"": ""uaa-id-213"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""ebfb72fa-76ab-4e93-8a02-2ae4ab74b354"",
        ""actee_type"": ""service_binding"",
        ""actee_name"": """",
        ""timestamp"": ""2016-09-02T11:52:19Z"",
        ""metadata"": {
          ""request"": {
            ""service_instance_guid"": ""a2268aa9-6b19-4603-84f0-aa0bbe2485f0"",
            ""app_guid"": ""a2268aa9-6b19-4603-84f0-aa0bbe2485f0""
          }
        },
        ""space_guid"": ""a2268aa9-6b19-4603-84f0-aa0bbe2485f0"",
        ""organization_guid"": ""a2268aa9-6b19-4603-84f0-aa0bbe2485f0""
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceBindingCreateEventsResponse> page = Utilities.DeserializePage<ListServiceBindingCreateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("a2268aa9-6b19-4603-84f0-aa0bbe2485f0", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/02f311b8-6f3c-472e-9b81-ab7a0deee590", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:19Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_binding.create", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-213", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("ebfb72fa-76ab-4e93-8a02-2ae4ab74b354", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_binding", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:19Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("a2268aa9-6b19-4603-84f0-aa0bbe2485f0", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("a2268aa9-6b19-4603-84f0-aa0bbe2485f0", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServicePlanDeleteEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""ad2d809a-7f58-4195-b5ce-04cfb045a698"",
        ""url"": ""/v2/events/02558662-1056-4530-bdbb-e5e5cc2d2a5c"",
        ""created_at"": ""2016-09-02T11:52:16Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_plan.delete"",
        ""actor"": ""e8bf4393-2da5-4676-91b9-920b1f4266fd"",
        ""actor_type"": ""service_broker"",
        ""actor_name"": ""name-1166"",
        ""actee"": ""31a7d7d4-e6eb-4b77-a6ac-e5dda3f574dc"",
        ""actee_type"": ""service_plan"",
        ""actee_name"": ""name-1167"",
        ""timestamp"": ""2016-09-02T11:52:16Z"",
        ""metadata"": {

        },
        ""space_guid"": """",
        ""organization_guid"": """"
      }
    }
  ]
}";

            PagedResponseCollection<ListServicePlanDeleteEventsResponse> page = Utilities.DeserializePage<ListServicePlanDeleteEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("ad2d809a-7f58-4195-b5ce-04cfb045a698", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/02558662-1056-4530-bdbb-e5e5cc2d2a5c", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:16Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_plan.delete", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("e8bf4393-2da5-4676-91b9-920b1f4266fd", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("service_broker", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("name-1166", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("31a7d7d4-e6eb-4b77-a6ac-e5dda3f574dc", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_plan", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1167", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:16Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListAppExitedEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""315d12f5-8d2a-423f-b396-5dc08e3e9dae"",
        ""url"": ""/v2/events/e2b382c8-26f1-4f74-ba22-0059b2fe6cbb"",
        ""created_at"": ""2016-09-02T11:52:16Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""app.crash"",
        ""actor"": ""121acc1d-a973-4d8d-b48f-50ba5f516f48"",
        ""actor_type"": ""app"",
        ""actor_name"": ""name-1106"",
        ""actee"": ""121acc1d-a973-4d8d-b48f-50ba5f516f48"",
        ""actee_type"": ""app"",
        ""actee_name"": ""name-1106"",
        ""timestamp"": ""2016-09-02T11:52:16Z"",
        ""metadata"": {
          ""instance"": 0,
          ""index"": 1,
          ""exit_status"": ""1"",
          ""exit_description"": ""out of memory"",
          ""reason"": ""crashed""
        },
        ""space_guid"": ""315d12f5-8d2a-423f-b396-5dc08e3e9dae"",
        ""organization_guid"": ""315d12f5-8d2a-423f-b396-5dc08e3e9dae""
      }
    }
  ]
}";

            PagedResponseCollection<ListAppExitedEventsResponse> page = Utilities.DeserializePage<ListAppExitedEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("315d12f5-8d2a-423f-b396-5dc08e3e9dae", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/e2b382c8-26f1-4f74-ba22-0059b2fe6cbb", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:16Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("app.crash", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("121acc1d-a973-4d8d-b48f-50ba5f516f48", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("app", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("name-1106", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("121acc1d-a973-4d8d-b48f-50ba5f516f48", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("app", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1106", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:16Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("315d12f5-8d2a-423f-b396-5dc08e3e9dae", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("315d12f5-8d2a-423f-b396-5dc08e3e9dae", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceKeyDeleteEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""fcea00ac-0e23-49fc-9261-02e4b6362f41"",
        ""url"": ""/v2/events/011fa259-ca76-4810-b51a-d1d3c533603d"",
        ""created_at"": ""2016-09-02T11:52:21Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_key.delete"",
        ""actor"": ""uaa-id-229"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""7cdd09ac-ce61-4a4f-a02f-bb30ebb68af9"",
        ""actee_type"": ""service_key"",
        ""actee_name"": ""name-1970"",
        ""timestamp"": ""2016-09-02T11:52:21Z"",
        ""metadata"": {
          ""request"": {

          }
        },
        ""space_guid"": ""fcea00ac-0e23-49fc-9261-02e4b6362f41"",
        ""organization_guid"": ""fcea00ac-0e23-49fc-9261-02e4b6362f41""
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceKeyDeleteEventsResponse> page = Utilities.DeserializePage<ListServiceKeyDeleteEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("fcea00ac-0e23-49fc-9261-02e4b6362f41", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/011fa259-ca76-4810-b51a-d1d3c533603d", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_key.delete", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-229", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("7cdd09ac-ce61-4a4f-a02f-bb30ebb68af9", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_key", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1970", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("fcea00ac-0e23-49fc-9261-02e4b6362f41", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("fcea00ac-0e23-49fc-9261-02e4b6362f41", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListUserProvidedServiceInstanceUpdateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""18b2fc2c-c2e5-4cd1-aeb0-a1f3cf4b21fa"",
        ""url"": ""/v2/events/a884169f-647d-4b35-8f47-7c7e456fe93e"",
        ""created_at"": ""2016-09-02T11:52:20Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.user_provided_service_instance.update"",
        ""actor"": ""uaa-id-221"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""8d718ecc-3c1f-4f83-9671-d8068471ea54"",
        ""actee_type"": ""user_provided_service_instance"",
        ""actee_name"": ""name-1848"",
        ""timestamp"": ""2016-09-02T11:52:20Z"",
        ""metadata"": {
          ""request"": {
            ""credentials"": ""[REDACTED]""
          }
        },
        ""space_guid"": ""18b2fc2c-c2e5-4cd1-aeb0-a1f3cf4b21fa"",
        ""organization_guid"": ""18b2fc2c-c2e5-4cd1-aeb0-a1f3cf4b21fa""
      }
    }
  ]
}";

            PagedResponseCollection<ListUserProvidedServiceInstanceUpdateEventsResponse> page = Utilities.DeserializePage<ListUserProvidedServiceInstanceUpdateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("18b2fc2c-c2e5-4cd1-aeb0-a1f3cf4b21fa", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/a884169f-647d-4b35-8f47-7c7e456fe93e", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:20Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.user_provided_service_instance.update", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-221", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("8d718ecc-3c1f-4f83-9671-d8068471ea54", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("user_provided_service_instance", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1848", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:20Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("18b2fc2c-c2e5-4cd1-aeb0-a1f3cf4b21fa", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("18b2fc2c-c2e5-4cd1-aeb0-a1f3cf4b21fa", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListUserProvidedServiceInstanceDeleteEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""ac30249d-3eb7-4f93-9326-e49ec6ccc962"",
        ""url"": ""/v2/events/34a74e7e-f0b2-4682-b913-9418e7f18884"",
        ""created_at"": ""2016-09-02T11:52:16Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.user_provided_service_instance.delete"",
        ""actor"": ""uaa-id-164"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""b644f5a7-524b-4919-b68a-a0989d98122f"",
        ""actee_type"": ""user_provided_service_instance"",
        ""actee_name"": ""name-1048"",
        ""timestamp"": ""2016-09-02T11:52:16Z"",
        ""metadata"": {
          ""request"": {

          }
        },
        ""space_guid"": ""ac30249d-3eb7-4f93-9326-e49ec6ccc962"",
        ""organization_guid"": ""ac30249d-3eb7-4f93-9326-e49ec6ccc962""
      }
    }
  ]
}";

            PagedResponseCollection<ListUserProvidedServiceInstanceDeleteEventsResponse> page = Utilities.DeserializePage<ListUserProvidedServiceInstanceDeleteEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("ac30249d-3eb7-4f93-9326-e49ec6ccc962", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/34a74e7e-f0b2-4682-b913-9418e7f18884", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:16Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.user_provided_service_instance.delete", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-164", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("b644f5a7-524b-4919-b68a-a0989d98122f", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("user_provided_service_instance", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1048", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:16Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("ac30249d-3eb7-4f93-9326-e49ec6ccc962", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("ac30249d-3eb7-4f93-9326-e49ec6ccc962", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListAppStartEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""4cacab5c-c7e7-45f9-ac05-6a831160feff"",
        ""url"": ""/v2/events/82a17cc6-462e-4b81-a220-9fefcabc5ea9"",
        ""created_at"": ""2016-09-02T11:52:18Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.app.start"",
        ""actor"": ""uaa-id-197"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""61f635fb-e38f-4021-9f31-37a2cd457223"",
        ""actee_type"": ""v3-app"",
        ""actee_name"": ""name-1520"",
        ""timestamp"": ""2016-09-02T11:52:18Z"",
        ""metadata"": {

        },
        ""space_guid"": ""4cacab5c-c7e7-45f9-ac05-6a831160feff"",
        ""organization_guid"": ""4cacab5c-c7e7-45f9-ac05-6a831160feff""
      }
    }
  ]
}";

            PagedResponseCollection<ListAppStartEventsResponse> page = Utilities.DeserializePage<ListAppStartEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("4cacab5c-c7e7-45f9-ac05-6a831160feff", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/82a17cc6-462e-4b81-a220-9fefcabc5ea9", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:18Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.app.start", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-197", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("61f635fb-e38f-4021-9f31-37a2cd457223", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("v3-app", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1520", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:18Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("4cacab5c-c7e7-45f9-ac05-6a831160feff", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("4cacab5c-c7e7-45f9-ac05-6a831160feff", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceKeyCreateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""de4eaba3-0cfe-459c-bf2c-4f08e7d102d8"",
        ""url"": ""/v2/events/653d62c2-befb-402b-bbfe-a2d151783385"",
        ""created_at"": ""2016-09-02T11:52:21Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_key.create"",
        ""actor"": ""uaa-id-231"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""61899271-2107-4acb-9e67-337fb1c09f2e"",
        ""actee_type"": ""service_key"",
        ""actee_name"": ""name-2001"",
        ""timestamp"": ""2016-09-02T11:52:21Z"",
        ""metadata"": {
          ""request"": {
            ""service_instance_guid"": ""de4eaba3-0cfe-459c-bf2c-4f08e7d102d8"",
            ""name"": ""name-2001""
          }
        },
        ""space_guid"": ""de4eaba3-0cfe-459c-bf2c-4f08e7d102d8"",
        ""organization_guid"": ""de4eaba3-0cfe-459c-bf2c-4f08e7d102d8""
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceKeyCreateEventsResponse> page = Utilities.DeserializePage<ListServiceKeyCreateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("de4eaba3-0cfe-459c-bf2c-4f08e7d102d8", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/653d62c2-befb-402b-bbfe-a2d151783385", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_key.create", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-231", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("61899271-2107-4acb-9e67-337fb1c09f2e", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_key", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-2001", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("de4eaba3-0cfe-459c-bf2c-4f08e7d102d8", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("de4eaba3-0cfe-459c-bf2c-4f08e7d102d8", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListAllEventsResponse()
        {
            string json = @"{
  ""total_results"": 3,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""7f01d86f-497d-4b37-bcdc-fd457dda17f3"",
        ""url"": ""/v2/events/2400b28c-f300-4e56-9fc1-24bfdf971883"",
        ""created_at"": ""2016-09-02T11:52:15Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""name-976"",
        ""actor"": ""b7297cfd-3489-481e-bb42-e947b4b4828d"",
        ""actor_type"": ""name-977"",
        ""actor_name"": ""name-978"",
        ""actee"": ""066cb1e7-c520-4755-9a01-433fb1efdc00"",
        ""actee_type"": ""name-979"",
        ""actee_name"": ""name-980"",
        ""timestamp"": ""2016-09-02T11:52:15Z"",
        ""metadata"": {

        },
        ""space_guid"": ""7f01d86f-497d-4b37-bcdc-fd457dda17f3"",
        ""organization_guid"": ""7f01d86f-497d-4b37-bcdc-fd457dda17f3""
      }
    },
    {
      ""metadata"": {
        ""guid"": ""7f01d86f-497d-4b37-bcdc-fd457dda17f3"",
        ""url"": ""/v2/events/ae0a293d-25e1-4586-9aab-b923e3ff8429"",
        ""created_at"": ""2016-09-02T11:52:15Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""name-984"",
        ""actor"": ""52c1a11b-b7de-47ba-9ec6-53a65a9199ee"",
        ""actor_type"": ""name-985"",
        ""actor_name"": ""name-986"",
        ""actee"": ""5f9c9e11-e321-4da3-908a-92b4a6e15202"",
        ""actee_type"": ""name-987"",
        ""actee_name"": ""name-988"",
        ""timestamp"": ""2016-09-02T11:52:15Z"",
        ""metadata"": {

        },
        ""space_guid"": ""7f01d86f-497d-4b37-bcdc-fd457dda17f3"",
        ""organization_guid"": ""7f01d86f-497d-4b37-bcdc-fd457dda17f3""
      }
    },
    {
      ""metadata"": {
        ""guid"": ""7f01d86f-497d-4b37-bcdc-fd457dda17f3"",
        ""url"": ""/v2/events/9639c7a3-d16f-44f1-b3da-d74b1473c6f4"",
        ""created_at"": ""2016-09-02T11:52:15Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""name-992"",
        ""actor"": ""0769122a-2055-4c8d-9575-c2317531fe36"",
        ""actor_type"": ""name-993"",
        ""actor_name"": ""name-994"",
        ""actee"": ""ab7df71b-97f2-41a9-8a1d-05acd839330c"",
        ""actee_type"": ""name-995"",
        ""actee_name"": ""name-996"",
        ""timestamp"": ""2016-09-02T11:52:15Z"",
        ""metadata"": {

        },
        ""space_guid"": ""7f01d86f-497d-4b37-bcdc-fd457dda17f3"",
        ""organization_guid"": ""7f01d86f-497d-4b37-bcdc-fd457dda17f3""
      }
    }
  ]
}";

            PagedResponseCollection<ListAllEventsResponse> page = Utilities.DeserializePage<ListAllEventsResponse>(json, null);

            Assert.AreEqual("3", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("7f01d86f-497d-4b37-bcdc-fd457dda17f3", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/2400b28c-f300-4e56-9fc1-24bfdf971883", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:15Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("name-976", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("b7297cfd-3489-481e-bb42-e947b4b4828d", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("name-977", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("name-978", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("066cb1e7-c520-4755-9a01-433fb1efdc00", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("name-979", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-980", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:15Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("7f01d86f-497d-4b37-bcdc-fd457dda17f3", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("7f01d86f-497d-4b37-bcdc-fd457dda17f3", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
            Assert.AreEqual("7f01d86f-497d-4b37-bcdc-fd457dda17f3", TestUtil.ToTestableString(page[1].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/ae0a293d-25e1-4586-9aab-b923e3ff8429", TestUtil.ToTestableString(page[1].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:15Z", TestUtil.ToTestableString(page[1].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[1].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("name-984", TestUtil.ToTestableString(page[1].Type), true);
            Assert.AreEqual("52c1a11b-b7de-47ba-9ec6-53a65a9199ee", TestUtil.ToTestableString(page[1].Actor), true);
            Assert.AreEqual("name-985", TestUtil.ToTestableString(page[1].ActorType), true);
            Assert.AreEqual("name-986", TestUtil.ToTestableString(page[1].ActorName), true);
            Assert.AreEqual("5f9c9e11-e321-4da3-908a-92b4a6e15202", TestUtil.ToTestableString(page[1].Actee), true);
            Assert.AreEqual("name-987", TestUtil.ToTestableString(page[1].ActeeType), true);
            Assert.AreEqual("name-988", TestUtil.ToTestableString(page[1].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:15Z", TestUtil.ToTestableString(page[1].Timestamp), true);
            Assert.AreEqual("7f01d86f-497d-4b37-bcdc-fd457dda17f3", TestUtil.ToTestableString(page[1].SpaceGuid), true);
            Assert.AreEqual("7f01d86f-497d-4b37-bcdc-fd457dda17f3", TestUtil.ToTestableString(page[1].OrganizationGuid), true);
            Assert.AreEqual("7f01d86f-497d-4b37-bcdc-fd457dda17f3", TestUtil.ToTestableString(page[2].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/9639c7a3-d16f-44f1-b3da-d74b1473c6f4", TestUtil.ToTestableString(page[2].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:15Z", TestUtil.ToTestableString(page[2].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[2].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("name-992", TestUtil.ToTestableString(page[2].Type), true);
            Assert.AreEqual("0769122a-2055-4c8d-9575-c2317531fe36", TestUtil.ToTestableString(page[2].Actor), true);
            Assert.AreEqual("name-993", TestUtil.ToTestableString(page[2].ActorType), true);
            Assert.AreEqual("name-994", TestUtil.ToTestableString(page[2].ActorName), true);
            Assert.AreEqual("ab7df71b-97f2-41a9-8a1d-05acd839330c", TestUtil.ToTestableString(page[2].Actee), true);
            Assert.AreEqual("name-995", TestUtil.ToTestableString(page[2].ActeeType), true);
            Assert.AreEqual("name-996", TestUtil.ToTestableString(page[2].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:15Z", TestUtil.ToTestableString(page[2].Timestamp), true);
            Assert.AreEqual("7f01d86f-497d-4b37-bcdc-fd457dda17f3", TestUtil.ToTestableString(page[2].SpaceGuid), true);
            Assert.AreEqual("7f01d86f-497d-4b37-bcdc-fd457dda17f3", TestUtil.ToTestableString(page[2].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListRouteCreateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""234b0cad-c3f1-4ab7-b214-9b22036cc4b4"",
        ""url"": ""/v2/events/8674bdf5-9301-4b36-993d-662a9892acb1"",
        ""created_at"": ""2016-09-02T11:52:20Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.route.create"",
        ""actor"": ""uaa-id-215"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""832cdaf3-5871-4a12-a704-5f4de69fadb9"",
        ""actee_type"": ""route"",
        ""actee_name"": ""host-24"",
        ""timestamp"": ""2016-09-02T11:52:20Z"",
        ""metadata"": {
          ""request"": {
            ""host"": ""dora"",
            ""domain_guid"": ""234b0cad-c3f1-4ab7-b214-9b22036cc4b4"",
            ""space_guid"": ""234b0cad-c3f1-4ab7-b214-9b22036cc4b4""
          }
        },
        ""space_guid"": ""234b0cad-c3f1-4ab7-b214-9b22036cc4b4"",
        ""organization_guid"": ""234b0cad-c3f1-4ab7-b214-9b22036cc4b4""
      }
    }
  ]
}";

            PagedResponseCollection<ListRouteCreateEventsResponse> page = Utilities.DeserializePage<ListRouteCreateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("234b0cad-c3f1-4ab7-b214-9b22036cc4b4", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/8674bdf5-9301-4b36-993d-662a9892acb1", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:20Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.route.create", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-215", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("832cdaf3-5871-4a12-a704-5f4de69fadb9", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("route", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("host-24", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:20Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("234b0cad-c3f1-4ab7-b214-9b22036cc4b4", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("234b0cad-c3f1-4ab7-b214-9b22036cc4b4", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServicePlanVisibilityCreateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""08fe805b-e611-48e9-87c0-19d6b8f4f814"",
        ""url"": ""/v2/events/4d6a85eb-829a-422f-bbf1-f932be43f38e"",
        ""created_at"": ""2016-09-02T11:52:21Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_plan_visibility.create"",
        ""actor"": ""uaa-id-227"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""9e0c05a7-8049-4a23-8860-1be4877fefb4"",
        ""actee_type"": ""service_plan_visibility"",
        ""actee_name"": """",
        ""timestamp"": ""2016-09-02T11:52:21Z"",
        ""metadata"": {
          ""request"": {
            ""service_plan_guid"": ""08fe805b-e611-48e9-87c0-19d6b8f4f814"",
            ""organization_guid"": ""08fe805b-e611-48e9-87c0-19d6b8f4f814""
          }
        },
        ""space_guid"": """",
        ""organization_guid"": ""08fe805b-e611-48e9-87c0-19d6b8f4f814""
      }
    }
  ]
}";

            PagedResponseCollection<ListServicePlanVisibilityCreateEventsResponse> page = Utilities.DeserializePage<ListServicePlanVisibilityCreateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("08fe805b-e611-48e9-87c0-19d6b8f4f814", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/4d6a85eb-829a-422f-bbf1-f932be43f38e", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_plan_visibility.create", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-227", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("9e0c05a7-8049-4a23-8860-1be4877fefb4", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_plan_visibility", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("08fe805b-e611-48e9-87c0-19d6b8f4f814", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListEventsAssociatedWithAppSinceJanuary12014Response()
        {
            string json = @"{
  ""total_results"": 3,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""590f247e-5632-40b6-9bd7-029afb3f38b5"",
        ""url"": ""/v2/events/bcd6851e-6f90-4e3a-946d-51cd547feae3"",
        ""created_at"": ""2016-09-02T11:52:21Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.app.create"",
        ""actor"": ""uaa-id-233"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""fe7e6726-6543-4355-80fe-8d0c9a6b4a44"",
        ""actee_type"": ""app"",
        ""actee_name"": ""name-2026"",
        ""timestamp"": ""2016-09-02T11:52:21Z"",
        ""metadata"": {
          ""request"": {
            ""name"": ""new"",
            ""instances"": 1,
            ""memory"": 84,
            ""state"": ""STOPPED"",
            ""environment_json"": ""PRIVATE DATA HIDDEN"",
            ""docker_credentials_json"": ""PRIVATE DATA HIDDEN""
          }
        },
        ""space_guid"": ""590f247e-5632-40b6-9bd7-029afb3f38b5"",
        ""organization_guid"": ""590f247e-5632-40b6-9bd7-029afb3f38b5""
      }
    },
    {
      ""metadata"": {
        ""guid"": ""590f247e-5632-40b6-9bd7-029afb3f38b5"",
        ""url"": ""/v2/events/97d5794c-23b2-475e-a2b5-3b3b34fd5701"",
        ""created_at"": ""2016-09-02T11:52:21Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.app.update"",
        ""actor"": ""uaa-id-233"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""fe7e6726-6543-4355-80fe-8d0c9a6b4a44"",
        ""actee_type"": ""app"",
        ""actee_name"": ""name-2026"",
        ""timestamp"": ""2016-09-02T11:52:21Z"",
        ""metadata"": {
          ""request"": {
            ""name"": ""new"",
            ""instances"": 1,
            ""memory"": 84,
            ""state"": ""STOPPED"",
            ""environment_json"": ""PRIVATE DATA HIDDEN"",
            ""docker_credentials_json"": ""PRIVATE DATA HIDDEN""
          }
        },
        ""space_guid"": ""590f247e-5632-40b6-9bd7-029afb3f38b5"",
        ""organization_guid"": ""590f247e-5632-40b6-9bd7-029afb3f38b5""
      }
    },
    {
      ""metadata"": {
        ""guid"": ""590f247e-5632-40b6-9bd7-029afb3f38b5"",
        ""url"": ""/v2/events/70a1bdb6-7136-4563-b4c9-d1b470f09ff4"",
        ""created_at"": ""2016-09-02T11:52:21Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.app.delete-request"",
        ""actor"": ""uaa-id-233"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""fe7e6726-6543-4355-80fe-8d0c9a6b4a44"",
        ""actee_type"": ""app"",
        ""actee_name"": ""name-2026"",
        ""timestamp"": ""2016-09-02T11:52:21Z"",
        ""metadata"": {
          ""request"": {
            ""recursive"": false
          }
        },
        ""space_guid"": ""590f247e-5632-40b6-9bd7-029afb3f38b5"",
        ""organization_guid"": ""590f247e-5632-40b6-9bd7-029afb3f38b5""
      }
    }
  ]
}";

            PagedResponseCollection<ListEventsAssociatedWithAppSinceJanuary12014Response> page = Utilities.DeserializePage<ListEventsAssociatedWithAppSinceJanuary12014Response>(json, null);

            Assert.AreEqual("3", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("590f247e-5632-40b6-9bd7-029afb3f38b5", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/bcd6851e-6f90-4e3a-946d-51cd547feae3", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.app.create", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-233", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("fe7e6726-6543-4355-80fe-8d0c9a6b4a44", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("app", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-2026", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("590f247e-5632-40b6-9bd7-029afb3f38b5", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("590f247e-5632-40b6-9bd7-029afb3f38b5", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
            Assert.AreEqual("590f247e-5632-40b6-9bd7-029afb3f38b5", TestUtil.ToTestableString(page[1].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/97d5794c-23b2-475e-a2b5-3b3b34fd5701", TestUtil.ToTestableString(page[1].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[1].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[1].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.app.update", TestUtil.ToTestableString(page[1].Type), true);
            Assert.AreEqual("uaa-id-233", TestUtil.ToTestableString(page[1].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[1].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[1].ActorName), true);
            Assert.AreEqual("fe7e6726-6543-4355-80fe-8d0c9a6b4a44", TestUtil.ToTestableString(page[1].Actee), true);
            Assert.AreEqual("app", TestUtil.ToTestableString(page[1].ActeeType), true);
            Assert.AreEqual("name-2026", TestUtil.ToTestableString(page[1].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[1].Timestamp), true);
            Assert.AreEqual("590f247e-5632-40b6-9bd7-029afb3f38b5", TestUtil.ToTestableString(page[1].SpaceGuid), true);
            Assert.AreEqual("590f247e-5632-40b6-9bd7-029afb3f38b5", TestUtil.ToTestableString(page[1].OrganizationGuid), true);
            Assert.AreEqual("590f247e-5632-40b6-9bd7-029afb3f38b5", TestUtil.ToTestableString(page[2].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/70a1bdb6-7136-4563-b4c9-d1b470f09ff4", TestUtil.ToTestableString(page[2].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[2].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[2].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.app.delete-request", TestUtil.ToTestableString(page[2].Type), true);
            Assert.AreEqual("uaa-id-233", TestUtil.ToTestableString(page[2].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[2].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[2].ActorName), true);
            Assert.AreEqual("fe7e6726-6543-4355-80fe-8d0c9a6b4a44", TestUtil.ToTestableString(page[2].Actee), true);
            Assert.AreEqual("app", TestUtil.ToTestableString(page[2].ActeeType), true);
            Assert.AreEqual("name-2026", TestUtil.ToTestableString(page[2].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[2].Timestamp), true);
            Assert.AreEqual("590f247e-5632-40b6-9bd7-029afb3f38b5", TestUtil.ToTestableString(page[2].SpaceGuid), true);
            Assert.AreEqual("590f247e-5632-40b6-9bd7-029afb3f38b5", TestUtil.ToTestableString(page[2].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceInstanceCreateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""1f95d45d-f577-4dcd-a3ec-1022e7c634aa"",
        ""url"": ""/v2/events/ae4b82d0-0bea-4563-9640-b9dfd8d494b9"",
        ""created_at"": ""2016-09-02T11:52:17Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_instance.create"",
        ""actor"": ""uaa-id-187"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""2efb215d-b968-4394-9e20-50f6e02e33c9"",
        ""actee_type"": ""service_instance"",
        ""actee_name"": ""name-1380"",
        ""timestamp"": ""2016-09-02T11:52:17Z"",
        ""metadata"": {
          ""request"": {
            ""name"": ""name-1380"",
            ""service_plan_guid"": ""1f95d45d-f577-4dcd-a3ec-1022e7c634aa"",
            ""space_guid"": ""1f95d45d-f577-4dcd-a3ec-1022e7c634aa"",
            ""parameters"": ""[PRIVATE DATA HIDDEN]""
          }
        },
        ""space_guid"": ""1f95d45d-f577-4dcd-a3ec-1022e7c634aa"",
        ""organization_guid"": ""1f95d45d-f577-4dcd-a3ec-1022e7c634aa""
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceInstanceCreateEventsResponse> page = Utilities.DeserializePage<ListServiceInstanceCreateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("1f95d45d-f577-4dcd-a3ec-1022e7c634aa", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/ae4b82d0-0bea-4563-9640-b9dfd8d494b9", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:17Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_instance.create", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-187", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("2efb215d-b968-4394-9e20-50f6e02e33c9", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_instance", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1380", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:17Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("1f95d45d-f577-4dcd-a3ec-1022e7c634aa", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("1f95d45d-f577-4dcd-a3ec-1022e7c634aa", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceDeleteEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""14c1809e-1f2f-4f06-9e17-32e3c68b7966"",
        ""url"": ""/v2/events/cce28a65-a8ca-43cf-a2fc-60b1e84cdbea"",
        ""created_at"": ""2016-09-02T11:52:20Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service.delete"",
        ""actor"": ""c72400e2-92c4-41ac-b204-a61377581893"",
        ""actor_type"": ""service_broker"",
        ""actor_name"": ""name-1793"",
        ""actee"": ""2405d04c-08f1-4a31-a943-75c722031dbf"",
        ""actee_type"": ""service"",
        ""actee_name"": ""label-59"",
        ""timestamp"": ""2016-09-02T11:52:20Z"",
        ""metadata"": {

        },
        ""space_guid"": """",
        ""organization_guid"": """"
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceDeleteEventsResponse> page = Utilities.DeserializePage<ListServiceDeleteEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("14c1809e-1f2f-4f06-9e17-32e3c68b7966", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/cce28a65-a8ca-43cf-a2fc-60b1e84cdbea", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:20Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service.delete", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("c72400e2-92c4-41ac-b204-a61377581893", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("service_broker", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("name-1793", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("2405d04c-08f1-4a31-a943-75c722031dbf", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("label-59", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:20Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServicePlanCreateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""883fbeed-c92b-4ab5-8db7-071c0c1ed5e0"",
        ""url"": ""/v2/events/d1178aa2-4797-49dd-a9bc-173eeb802bc5"",
        ""created_at"": ""2016-09-02T11:52:18Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_plan.create"",
        ""actor"": ""23fa808e-9ec8-4817-b115-5dd489cc0f71"",
        ""actor_type"": ""service_broker"",
        ""actor_name"": ""name-1575"",
        ""actee"": ""guid"",
        ""actee_type"": ""service_plan"",
        ""actee_name"": ""plan-name"",
        ""timestamp"": ""2016-09-02T11:52:18Z"",
        ""metadata"": {
          ""name"": ""plan-name"",
          ""free"": true,
          ""description"": ""A plan"",
          ""service_guid"": ""883fbeed-c92b-4ab5-8db7-071c0c1ed5e0"",
          ""extra"": null,
          ""unique_id"": ""guid"",
          ""public"": true,
          ""active"": true
        },
        ""space_guid"": """",
        ""organization_guid"": """"
      }
    }
  ]
}";

            PagedResponseCollection<ListServicePlanCreateEventsResponse> page = Utilities.DeserializePage<ListServicePlanCreateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("883fbeed-c92b-4ab5-8db7-071c0c1ed5e0", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/d1178aa2-4797-49dd-a9bc-173eeb802bc5", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:18Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_plan.create", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("23fa808e-9ec8-4817-b115-5dd489cc0f71", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("service_broker", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("name-1575", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("guid", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_plan", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("plan-name", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:18Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceBrokerCreateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""bd81caf2-56bf-4d34-bbee-c6d3487ad97e"",
        ""url"": ""/v2/events/dc529de9-8da4-43bd-a89b-e4567c8f353f"",
        ""created_at"": ""2016-09-02T11:52:19Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_broker.create"",
        ""actor"": ""uaa-id-203"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""0174015a-a688-4f4a-a39b-eecbac8a9b5d"",
        ""actee_type"": ""service_broker"",
        ""actee_name"": ""pancake broker"",
        ""timestamp"": ""2016-09-02T11:52:19Z"",
        ""metadata"": {
          ""request"": {
            ""name"": ""pancake broker"",
            ""broker_url"": ""http://www.pancakes.com"",
            ""auth_username"": ""panda"",
            ""auth_password"": ""[REDACTED]""
          }
        },
        ""space_guid"": """",
        ""organization_guid"": """"
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceBrokerCreateEventsResponse> page = Utilities.DeserializePage<ListServiceBrokerCreateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("bd81caf2-56bf-4d34-bbee-c6d3487ad97e", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/dc529de9-8da4-43bd-a89b-e4567c8f353f", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:19Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_broker.create", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-203", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("0174015a-a688-4f4a-a39b-eecbac8a9b5d", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_broker", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("pancake broker", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:19Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServicePlanUpdateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""abd9c686-676d-45a8-be7e-f6eef1a96c0d"",
        ""url"": ""/v2/events/d81ed85f-2350-4d1e-aef7-de89246baa10"",
        ""created_at"": ""2016-09-02T11:52:16Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_plan.update"",
        ""actor"": ""6089c029-22a2-4a79-88af-e452c083e578"",
        ""actor_type"": ""service_broker"",
        ""actor_name"": ""name-1217"",
        ""actee"": ""660c29ac-4808-49ea-ae8d-4b8b0084e54f"",
        ""actee_type"": ""service_plan"",
        ""actee_name"": ""new name"",
        ""timestamp"": ""2016-09-02T11:52:16Z"",
        ""metadata"": {
          ""name"": ""new name""
        },
        ""space_guid"": """",
        ""organization_guid"": """"
      }
    }
  ]
}";

            PagedResponseCollection<ListServicePlanUpdateEventsResponse> page = Utilities.DeserializePage<ListServicePlanUpdateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("abd9c686-676d-45a8-be7e-f6eef1a96c0d", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/d81ed85f-2350-4d1e-aef7-de89246baa10", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:16Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_plan.update", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("6089c029-22a2-4a79-88af-e452c083e578", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("service_broker", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("name-1217", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("660c29ac-4808-49ea-ae8d-4b8b0084e54f", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_plan", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("new name", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:16Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListRouteDeleteEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""29609c4c-8b5f-4ade-b272-989f939b6b95"",
        ""url"": ""/v2/events/eae461f0-9932-4326-85ce-4256d3d1e764"",
        ""created_at"": ""2016-09-02T11:52:18Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.route.delete-request"",
        ""actor"": ""uaa-id-199"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""a7bc0d76-3437-41e5-adfd-9f7f9d4d9839"",
        ""actee_type"": ""route"",
        ""actee_name"": ""host-23"",
        ""timestamp"": ""2016-09-02T11:52:18Z"",
        ""metadata"": {
          ""request"": {
            ""recursive"": true
          }
        },
        ""space_guid"": ""29609c4c-8b5f-4ade-b272-989f939b6b95"",
        ""organization_guid"": ""29609c4c-8b5f-4ade-b272-989f939b6b95""
      }
    }
  ]
}";

            PagedResponseCollection<ListRouteDeleteEventsResponse> page = Utilities.DeserializePage<ListRouteDeleteEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("29609c4c-8b5f-4ade-b272-989f939b6b95", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/eae461f0-9932-4326-85ce-4256d3d1e764", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:18Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.route.delete-request", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-199", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("a7bc0d76-3437-41e5-adfd-9f7f9d4d9839", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("route", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("host-23", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:18Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("29609c4c-8b5f-4ade-b272-989f939b6b95", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("29609c4c-8b5f-4ade-b272-989f939b6b95", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListServiceBindingDeleteEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""a4b721f1-bf0b-4867-a020-1ad0ad26ca1b"",
        ""url"": ""/v2/events/50c7f2fa-49e1-4f1a-85c7-e424f2a38cd1"",
        ""created_at"": ""2016-09-02T11:52:22Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.service_binding.delete"",
        ""actor"": ""uaa-id-239"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""0da16913-4773-4051-8a74-abc33cdf4ddd"",
        ""actee_type"": ""service_binding"",
        ""actee_name"": """",
        ""timestamp"": ""2016-09-02T11:52:22Z"",
        ""metadata"": {
          ""request"": {

          }
        },
        ""space_guid"": ""a4b721f1-bf0b-4867-a020-1ad0ad26ca1b"",
        ""organization_guid"": ""a4b721f1-bf0b-4867-a020-1ad0ad26ca1b""
      }
    }
  ]
}";

            PagedResponseCollection<ListServiceBindingDeleteEventsResponse> page = Utilities.DeserializePage<ListServiceBindingDeleteEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("a4b721f1-bf0b-4867-a020-1ad0ad26ca1b", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/50c7f2fa-49e1-4f1a-85c7-e424f2a38cd1", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:22Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.service_binding.delete", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-239", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("0da16913-4773-4051-8a74-abc33cdf4ddd", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("service_binding", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:22Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("a4b721f1-bf0b-4867-a020-1ad0ad26ca1b", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("a4b721f1-bf0b-4867-a020-1ad0ad26ca1b", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListAppUpdateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""c180a8e6-83a9-4463-8167-3f528779a752"",
        ""url"": ""/v2/events/53d22e02-aff0-46ee-b989-d53edad61b1a"",
        ""created_at"": ""2016-09-02T11:52:21Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.app.update"",
        ""actor"": ""uaa-id-235"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""8643374d-ce55-4c84-984d-10c78131af6f"",
        ""actee_type"": ""app"",
        ""actee_name"": ""name-2056"",
        ""timestamp"": ""2016-09-02T11:52:21Z"",
        ""metadata"": {
          ""request"": {
            ""name"": ""new"",
            ""instances"": 1,
            ""memory"": 84,
            ""state"": ""STOPPED"",
            ""environment_json"": ""PRIVATE DATA HIDDEN"",
            ""docker_credentials_json"": ""PRIVATE DATA HIDDEN""
          }
        },
        ""space_guid"": ""c180a8e6-83a9-4463-8167-3f528779a752"",
        ""organization_guid"": ""c180a8e6-83a9-4463-8167-3f528779a752""
      }
    }
  ]
}";

            PagedResponseCollection<ListAppUpdateEventsResponse> page = Utilities.DeserializePage<ListAppUpdateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("c180a8e6-83a9-4463-8167-3f528779a752", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/53d22e02-aff0-46ee-b989-d53edad61b1a", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.app.update", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-235", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("8643374d-ce55-4c84-984d-10c78131af6f", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("app", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-2056", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:21Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("c180a8e6-83a9-4463-8167-3f528779a752", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("c180a8e6-83a9-4463-8167-3f528779a752", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListAppSshUnauthorizedEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""bc7bc8c4-c9c1-415d-8de0-165289f43003"",
        ""url"": ""/v2/events/8a486998-646a-4219-9952-39c33cfca602"",
        ""created_at"": ""2016-09-02T11:52:20Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.app.ssh-unauthorized"",
        ""actor"": ""uaa-id-223"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""60361c89-0284-4249-8952-a5ed6e79f76f"",
        ""actee_type"": ""app"",
        ""actee_name"": ""name-1876"",
        ""timestamp"": ""2016-09-02T11:52:20Z"",
        ""metadata"": {
          ""index"": 1
        },
        ""space_guid"": ""bc7bc8c4-c9c1-415d-8de0-165289f43003"",
        ""organization_guid"": ""bc7bc8c4-c9c1-415d-8de0-165289f43003""
      }
    }
  ]
}";

            PagedResponseCollection<ListAppSshUnauthorizedEventsResponse> page = Utilities.DeserializePage<ListAppSshUnauthorizedEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("bc7bc8c4-c9c1-415d-8de0-165289f43003", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/8a486998-646a-4219-9952-39c33cfca602", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:20Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.app.ssh-unauthorized", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-223", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("60361c89-0284-4249-8952-a5ed6e79f76f", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("app", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1876", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:20Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("bc7bc8c4-c9c1-415d-8de0-165289f43003", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("bc7bc8c4-c9c1-415d-8de0-165289f43003", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListSpaceUpdateEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""94d019ae-9751-4f81-b712-1a50efdcaab1"",
        ""url"": ""/v2/events/7d74519f-44fc-494d-b0b0-a8a74cf5154a"",
        ""created_at"": ""2016-09-02T11:52:17Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.space.update"",
        ""actor"": ""uaa-id-181"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""4b88f87b-d73d-4a95-936a-3a70a9205893"",
        ""actee_type"": ""space"",
        ""actee_name"": ""name-1295"",
        ""timestamp"": ""2016-09-02T11:52:17Z"",
        ""metadata"": {
          ""request"": {
            ""name"": ""outer space""
          }
        },
        ""space_guid"": ""94d019ae-9751-4f81-b712-1a50efdcaab1"",
        ""organization_guid"": ""94d019ae-9751-4f81-b712-1a50efdcaab1""
      }
    }
  ]
}";

            PagedResponseCollection<ListSpaceUpdateEventsResponse> page = Utilities.DeserializePage<ListSpaceUpdateEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("94d019ae-9751-4f81-b712-1a50efdcaab1", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/7d74519f-44fc-494d-b0b0-a8a74cf5154a", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:17Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.space.update", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-181", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("4b88f87b-d73d-4a95-936a-3a70a9205893", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("space", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1295", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:17Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("94d019ae-9751-4f81-b712-1a50efdcaab1", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("94d019ae-9751-4f81-b712-1a50efdcaab1", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }

        [TestMethod]
        public void TestListAppDeleteEventsResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""a0e2a539-b230-48cd-a9a1-a5b2fd80d8a1"",
        ""url"": ""/v2/events/5ec8b452-ac0f-4d06-95da-b61a46ab797e"",
        ""created_at"": ""2016-09-02T11:52:16Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""type"": ""audit.app.delete-request"",
        ""actor"": ""uaa-id-166"",
        ""actor_type"": ""user"",
        ""actor_name"": ""user@example.com"",
        ""actee"": ""46ff0c3f-d07f-4c5b-b1b3-2213efb66b88"",
        ""actee_type"": ""app"",
        ""actee_name"": ""name-1076"",
        ""timestamp"": ""2016-09-02T11:52:16Z"",
        ""metadata"": {
          ""request"": {
            ""recursive"": false
          }
        },
        ""space_guid"": ""a0e2a539-b230-48cd-a9a1-a5b2fd80d8a1"",
        ""organization_guid"": ""a0e2a539-b230-48cd-a9a1-a5b2fd80d8a1""
      }
    }
  ]
}";

            PagedResponseCollection<ListAppDeleteEventsResponse> page = Utilities.DeserializePage<ListAppDeleteEventsResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("a0e2a539-b230-48cd-a9a1-a5b2fd80d8a1", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/events/5ec8b452-ac0f-4d06-95da-b61a46ab797e", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-09-02T11:52:16Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("audit.app.delete-request", TestUtil.ToTestableString(page[0].Type), true);
            Assert.AreEqual("uaa-id-166", TestUtil.ToTestableString(page[0].Actor), true);
            Assert.AreEqual("user", TestUtil.ToTestableString(page[0].ActorType), true);
            Assert.AreEqual("user@example.com", TestUtil.ToTestableString(page[0].ActorName), true);
            Assert.AreEqual("46ff0c3f-d07f-4c5b-b1b3-2213efb66b88", TestUtil.ToTestableString(page[0].Actee), true);
            Assert.AreEqual("app", TestUtil.ToTestableString(page[0].ActeeType), true);
            Assert.AreEqual("name-1076", TestUtil.ToTestableString(page[0].ActeeName), true);
            Assert.AreEqual("2016-09-02T11:52:16Z", TestUtil.ToTestableString(page[0].Timestamp), true);
            Assert.AreEqual("a0e2a539-b230-48cd-a9a1-a5b2fd80d8a1", TestUtil.ToTestableString(page[0].SpaceGuid), true);
            Assert.AreEqual("a0e2a539-b230-48cd-a9a1-a5b2fd80d8a1", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
        }
    }
}
